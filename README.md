...
也可以从 [GitHub Releases](https://github.com/yangkaixin233/PaperMirror/releases) 下载 `papermirror-vscode-0.1.7.vsix`，然后通过以下命令安装：
...
| `paperMirror.maxBlocksPerRequest` | 单次 provider 请求最多包含多少个 block。 |
...
You can also download `papermirror-vscode-0.1.7.vsix` from [GitHub Releases](https://github.com/yangkaixin233/PaperMirror/releases), then install it with:
...
| `paperMirror.maxBlocksPerRequest` | Maximum blocks per provider request. |
...
| `paperMirror.requestTimeoutMs` | Timeout for each provider request. |
